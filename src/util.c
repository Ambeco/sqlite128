...
/* SQLITE_USE_UINT128 is detected in sqliteInt.h (it needs to be visible
** before sqliteInt128.h is included there, to pick sqlite3_uint128's
** representation) and is available here as a side effect of the
** #include "sqliteInt.h" above. */
...
/*
** Transform a UTF-8 integer literal, in either decimal or (unless
** SQLITE_OMIT_HEX_INTEGER) hexadecimal, into a signed 128-bit integer.
** This is the Phase 6h, 128-bit analog of sqlite3DecOrHexToI64() just
** above, used only to parse SQL integer literal tokens (which are always
** UTF8, zero-terminated, and have no leading sign -- see codeInteger(),
** expr.c, and the Expr.u.zToken comment) once they're known to overflow
** 64 bits. Unlike sqlite3Atoi64()/sqlite3DecOrHexToI64() this does not
** need to handle UTF16, embedded whitespace, or a leading '+'/'-', since
** none of those appear in a literal token.
**
** Overflow is detected during accumulation (not just by counting digits)
** because a sufficiently long decimal run can overflow the 128-bit
** accumulator itself before the final magnitude comparison would catch
** it -- unlike the 64-bit case, where "19 or fewer digits always fits"
** is a cheap sufficient pre-check, the 128-bit analog ("38 or fewer
** digits always fits") is true but a 39+ digit input might already have
** wrapped past 2^128 during naive accumulation, which would make a
** post-hoc magnitude check meaningless.
**
** Returns:
**
**     0    Successful transformation. Fits in a 128-bit signed integer.
**     1    Excess text after the integer value
**     2    Integer too large for a 128-bit signed integer or malformed
**     3    Special case: magnitude is exactly 2^127 (fits only if negated)
*/
int sqlite3DecOrHexToI128(const char *z, sqlite3_uint128 *pOut){
#ifndef SQLITE_OMIT_HEX_INTEGER
  if( z[0]=='0' && (z[1]=='x' || z[1]=='X') ){
    sqlite3_uint128 sixteen = int128FromU64(16);
    sqlite3_uint128 u = int128FromU64(0);
    int i, k;
    for(i=2; z[i]=='0'; i++){}
    for(k=i; sqlite3Isxdigit(z[k]); k++){
      u = int128Add(int128MultiplyLow(u, sixteen),
                     int128FromU64(sqlite3HexToInt(z[k])));
    }
    *pOut = u;
    if( k-i>32 ) return 2;
    if( z[k]!=0 ) return 1;
...
#endif /* SQLITE_OMIT_HEX_INTEGER */
  {
    ...
    sqlite3_uint128 allOnes = int128Negate(int128FromU64(1));  /* 2^128-1 */
    sqlite3_uint128 boundary = int128ShiftLeft(int128FromU64(1), 127); /*2^127*/
    sqlite3_uint128 maxDiv10, maxRem10;
    sqlite3_uint128 u = int128FromU64(0);
    int overflow = 0;
    int hasDigit = 0;
    int i;

    int128DivModU(allOnes, ten, &maxDiv10, &maxRem10);
    for(i=0; sqlite3Isdigit(z[i]); i++){
      int d = z[i]-'0';
      hasDigit = 1;
      if( !overflow ){
        if( int128Compare(u, maxDiv10)>0
         || (int128Compare(u, maxDiv10)==0
              && d>(int)int128ToU64(maxRem10)) ){
          overflow = 1;
        }else{
          u = int128Add(int128MultiplyLow(u, ten), int128FromU64(d));
        }
      }
    }
    *pOut = u;
    if( !hasDigit ) return 2;    /* Not even a prefix looks like a number */
    if( overflow ) return 2;
    if( z[i]!=0 ) return 1;
    if( int128Compare(u, boundary)>0 ) return 2;
    if( int128Compare(u, boundary)==0 ) return 3;
    return 0;
  }
}
